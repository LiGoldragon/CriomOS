...
let
  ...
  # The two directories the module declares it owns. A whole NixOS system
  # carries many other tmpfiles rules, so this names ours rather than counting
  # everything.
  expectedTmpfilesRules = [
    "d ${settings.stateDirectoryPath} 0750 ${settings.user} ${settings.group} -"
    "d ${settings.runtimeDirectoryPath} 0750 ${settings.user} ${settings.group} -"
  ];
