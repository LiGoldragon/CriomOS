...
pkgs.runCommand "repository-receive-role-policy" { } ''
  ...
  printf '%s' ${lib.escapeShellArg hookText} | grep -F 'RepositoryPushObservation'
  printf '%s' ${lib.escapeShellArg hookText} | grep -F 'RepositoryCommitObservation'
  printf '%s' ${lib.escapeShellArg hookText} | grep -F 'RepositoryFileChange'
